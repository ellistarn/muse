...
under later noise, and that the failure is stark: conversations that should produce
...
Each observation strategy stores results in its own directory. Switching strategies leaves
other strategies' cached observations intact.
...
start immediately rather than arriving as a long tail. The conversation remains the unit of retry and storage. If any window returns a
fatal error, the conversation fails and will be re-observed next run.
