...
Hooks into `observeSession` in `distill.go`, scrubbing the formatted conversation before the LLM call.
...
After a successful distill, purge processed conversations from S3. Raw conversations are only needed until they're observed. Add `PurgeProcessedConversations` to the storage client.
...
| `internal/distill/distill.go` | Scrub conversation before observe, scrub skills after learn |
