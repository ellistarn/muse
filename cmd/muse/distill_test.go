...
func TestRunDistill_PropagatesRunError(t *testing.T) {
	...
	_, err := runDistill(ctx, &stdout, &stderr, store, &testutil.MockLLM{}, &testutil.MockLLM{}, &testutil.MockLLM{}, distill.Options{Limit: 100})
	if err == nil {
...
func TestRunDistill_PropagatesLearnError(t *testing.T) {
	...
	_, err := runDistill(ctx, &stdout, &stderr, store, nil, &testutil.MockLLM{Err: fmt.Errorf("learn failed")}, &testutil.MockLLM{}, distill.Options{Learn: true})
	if err == nil {
...
func TestRunDistill_SuccessfulRun(t *testing.T) {
	...
	_, err := runDistill(ctx, &stdout, &stderr, store, mockLLM, mockLLM, mockLLM, distill.Options{Limit: 100})
	if err != nil {
...
func TestRunDistill_SuccessfulLearn(t *testing.T) {
	...
	_, err := runDistill(ctx, &stdout, &stderr, store, nil, mockLLM, mockLLM, distill.Options{Learn: true})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.Contains(stdout.String(), "Muse distilled") {
